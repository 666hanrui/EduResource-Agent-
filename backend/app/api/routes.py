...
def build_router(
    ...
    async def create_student_interactive_classroom(
        ...
        if learning_path:
            current_step = next(
                (step for step in learning_path.steps if step.status in {"pending", "in_progress", "adjusted"}),
...
def _draft_interactive_package(
    *,
    ...
    rationale = ResourceRationale(
        ...
        source_trace=[f"interactive_classroom_job:{job_id}"],
        created_at=now,
    )
    placeholder = ResourceItem(
        ...
        content_json={"job_id": job_id, "status": "generating"},
        content_markdown=f"# {target_knowledge_name}互动课堂\n\n生成中：{learning_goal}",
        source_type="agent",
        source_url="",
...
def _map_openmaic_job(openmaic_job: dict) -> tuple[str, str | None]:
    raw_status = str(
        openmaic_job.get("status")
        or openmaic_job.get("state")
        or openmaic_job.get("jobStatus")
        or "running"
    ).lower()
    ...
    classroom_url = (
        openmaic_job.get("classroomUrl")
        or openmaic_job.get("classroom_url")
        or openmaic_job.get("url")
        or result.get("classroomUrl")
